...
describe('messageTemplate', () => {
  it('returns html and text given a certificate', () => {
    ...
    expect(extractContent(message.html)).toEqual(extractContent(expectedContentHtml));
    expect(extractContent(message.text)).toEqual(extractContent(expectedContentTxt));
    expect(extractContent(message.subject)).toEqual(extractContent(expectedContentSubject));
  });
  ...
  it('throws for invalid certificate', () => {
    expect(() => messageTemplate({foo: 'Invalid Data'})).toThrow('Fail to read data from certificate');
  });
});
