...
const cipher = (message, key) => {
  const cipherInstance = crypto.createCipheriv(
    ENCRYPTION_ALGORITHM,
    key,
    ENCRYPTION_IV
  );
  const encrypted =
    cipherInstance.update(message) + cipherInstance.final();
  return encrypted;
};

const decipher = (encrypted, key) => {
  const decipherInstance = crypto.createDecipheriv(
    ENCRYPTION_ALGORITHM,
    key,
    ENCRYPTION_IV
  );
  const decrypted =
    decipherInstance.update(encrypted) + decipherInstance.final();
  return decrypted;
};
